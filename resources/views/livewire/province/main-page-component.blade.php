                                                                ...
                                                                <table class="table table-hover">
                                                                    ...
                                                                    @foreach($cities as $k=>$item)
                                                                        <tr data-widget="expandable-table" aria-expanded="false">
                                                                            ...
                                                                            <td title="سهم برق از کل مصرف انرژی">@if($item->coeficent) {{$item->coeficent}}@else سهم برق از کل مصرف انرژی  @endif</td>
                                                                            <td>@if($item->latitude) {{$item->latitude}}@else عرض جغرافیایی @endif</td>
                                                                            <td>@if($item->longitude){{$item->longitude}} @else طول جغرافیایی @endif</td>
                                                                            <td>
                                                                        ...
                                                                        <tr class="expandable-body">
                                                                            <td>
                                                                                <div class="p-0">
                                                                                    <table class="table table-hover">
                                                                                        <tbody>
                                                                                        @foreach(\App\Models\Provinces::where('parent','!=',0)->where('parent',$k+1)->get() as $item1)
                                                                                            <tr>
                                                                                                <td>{{$item1->id}}-{{$item1->title}}</td>
                                                                                                <td>@if($item1->region)
                                                                                                    @if($item1->region == 1)  گرمایش زیاد@endif
                                                                                                    @if($item1->region == 2)  سرمایش زیاد@endif
                                                                                                    @if($item1->region == 3) گرمایش یا سرمایش متوسط@endif
                                                                                                    @if($item1->region == 4)  گرمایش یا سرمایش کم@endif
                                                                                                    @else منطقه ی جغرافیایی @endif</td>
                                                                                                <td>@if($item1->latitude) {{$item1->latitude}}@else عرض جغرافیایی @endif</td>
                                                                                                <td>@if($item1->longitude){{$item1->longitude}} @else طول جغرافیایی @endif</td>
                                                                                                <td>
...
{{--                @include('livewire.pwrconsum.main')--}}
    ...
    @include('livewire.province.modal')
